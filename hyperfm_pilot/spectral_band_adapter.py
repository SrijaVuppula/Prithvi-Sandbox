...
Both encoder-side integration questions this design originally flagged
(Conv3d vs Conv2d, config.json's patch_size shape) were resolved against
the real 100M/600M checkpoints -- see ControlNetBandAdapter's docstring.
"""
...
#   idx 282-290 : discrete SWIR, ~940-2258 nm        (9 bands, incl. two
#                 dual-gain duplicate pairs near 1250nm and 1618nm)
# Real values loaded from pace_band_wavelengths.py -- see
# build_pace_wavelength_array below.


def build_pace_wavelength_array(pace_swir_centers_nm=None):
    """
    Real 291-length PACE-OCI wavelength array, loaded directly from
    pace_band_wavelengths.py's BAND_WAVELENGTH_NM (confirmed exact values,
    Aug 24 wavelength-validation session -- read from an actual PACE L1B
    granule's sensor_band_parameters group, not approximated or guessed).
    Includes the real SWIR dual-gain duplicate pairs (e.g. two close-but-
    distinct values near 1250nm and near 1618nm), which a synthetic
    approximation would not reproduce.

    pace_swir_centers_nm: legacy override, only used if explicitly passed
    (e.g. for isolated unit tests) -- reconstructs the old approximated
    version instead of loading the real file. Not the default path.
    """
    if pace_swir_centers_nm is not None:
        blue = torch.linspace(314, 605, 119)
        red = torch.linspace(600, 895, 163)
        swir = torch.tensor(pace_swir_centers_nm, dtype=torch.float32)
    ...
    wavelengths = torch.tensor(BAND_WAVELENGTH_NM, dtype=torch.float32)
    assert wavelengths.numel() == 291, f"expected 291 bands, got {wavelengths.numel()}"
    return wavelengths
...
class SpectralHintEncoder(nn.Module):
    ...
    patch sizes (600M uses 14, not 16 -- see earlier "600M crash" bug note);
    always read patch_size from that backbone's own config.json, don't hardcode.
    """
    ...
    def __init__(self, in_bands: int, embed_dim: int, patch_size: int,
        ...
        # whatever patch_size that backbone uses (14 for 600M, 16 for 100M, etc.)
